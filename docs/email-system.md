...
> **Multi-domain note**: auth/email links (verify-email, password reset, etc.)
> are generated by `TenantService.frontendUrl()` — on a tenant host the link
> points at the tenant's own domain (e.g. `https://acme.saabiz.com/...`);
> on the platform host it uses `FRONTEND_URL` / `https://<platform-domain>`.
